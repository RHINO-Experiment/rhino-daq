...
class RtlSdrLogger:
    ...
    def get_avg_spectra(self, return_std=False):
        spectra = []
...
class SingleFrequencyObserverTester:
    def __init__(self, centre_freq=70e6, sample_rate = 2.048e6, averaging_time_per_sdr_sample = 1,
                 fft_length = 2048, sdr_gain = 0.0, spectrum_window='None', runtime=60, get_spectra_std=False, save_folder=''):
        self.sample_rate = sample_rate
        ...
        self.get_spectra_std = get_spectra_std
        pass
    ...
    def begin_observations(self, obs_title=None):
        ...
        sdr = RtlSdrLogger(self.averaging_time_per_sdr_sample, self.sample_rate, self.centre_freq,
                               fft_length=self.fft_length, sdr_gain_set = self.sdr_gain,
                               window_function=self.spectrum_window, averaging=True)
        t_0 = datetime.now()
        t_f = t_0 + timedelta(seconds=self.runtime)

        spectra = []
        spectra_std = []
        times = []
        if self.get_spectra_std:
            while t_0 < t_f:
                s, std = sdr.get_avg_spectra(return_std=True)
                spectra.append(s)
                spectra_std.append(std)
                t_0 = datetime.now()
                times.append(t_0)
        else:
            while t_0 < t_f:
                s = sdr.get_avg_spectra(return_std=False)
                spectra.append(s)
                t_0 = datetime.now()
                times.append(t_0)
        ...
        spectra = np.array(spectra)
        frequencies = np.linspace(self.centre_freq-self.sample_rate/2, 
        ...
        obs_group.create_dataset('spectra', data=spectra, dtype=spectra.dtype)
        obs_group.create_dataset('spectra_frequencies', data=frequencies, dtype=frequencies.dtype)
        t_ds = obs_group.create_dataset('times', data=times, dtype=times.dtype)
        epcoh_string = str(epoch.strftime('%Y_%m_%d_%H%M%S'))
        t_ds.attrs['epoch'] = epcoh_string
        if self.get_spectra_std:
            spectra_std = np.array(spectra_std)
            obs_group.create_dataset('spectra_std', data=spectra_std, dtype=spectra_std.dtype)
